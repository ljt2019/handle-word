...
public class XWPFTableHandler {
    ...
    /**
     * 所有匹配的值替换为对应的值key【匹配模板中的${key}】
     *
     * @param map 要替换的key-value集合
     * @return
     */
    public boolean replaceParagraphs(Map<String, String> map) {
        List<XWPFParagraph> paragraphs = getAllParagraphs();
        boolean flag = false;
        for (XWPFParagraph paragraph : paragraphs) {
            boolean currSuccessTag = new XWPFParagraphHandler(paragraph).replaceAll(map);
            flag = flag ? flag : currSuccessTag;
        }
        return flag;
    }
